...
{ev}</div>"""
    return f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><link rel="icon" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><rect width=%22100%22 height=%22100%22 rx=%2220%22 fill=%22%23ff6b35%22/><text y=%22.75em%22 x=%22.5em%22 font-size=%2270%22 font-weight=%22bold%22 font-family=%22Arial%22 fill=%22%23fff%22>P</text></svg>"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Peak — {now}</title></head>
<body style="font-family:-apple-system,sans-serif;max-width:820px;margin:0 auto;padding:2rem;background:#111;color:#eee">
<div style="border-bottom:1px solid #222;padding-bottom:1rem;margin-bottom:1.5rem"><span style="font-size:28px;font-weight:600;letter-spacing:-1px">pe<span style="color:#ff6b35">Λ</span>ker</span><span style="color:#444;font-size:13px;margin-left:12px">{len(events)} events · {now}</span></div>
